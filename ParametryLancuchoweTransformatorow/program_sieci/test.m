...
% handles    structure with handles and user data (see GUIDATA)
 ...
 if handles.afl==0&&handles.cu_al==0&&handles.FAZ1==0&&handles.FAZ3==0              %jeœli  nie wybrano materia³u na przewody
      errordlg('PODAJ MATERIA£ Z KTÓREGO ZBUDOWANE S¥ PRZEWODY I UK£AD ROZMIESZCZENIA LINII!!!', 'Error')
      return
 elseif  handles.FAZ1==0&&handles.FAZ3==0
      errordlg('PODAJ UK£AD ROZMIESZCZENIA LINII!!!', 'Error')
      return
 elseif handles.afl==0&&handles.cu_al==0
    errordlg('PODAJ MATERIA£ Z KTÓREGO ZBUDOWANE S¥ PRZEWODY!!!', 'Error')
    return
...
%Uw1<=30kV to biorê czwórnik dla U<=30kV i tak samo dla Uw1
if handles.Uw1<=30
...
if handles.Sn2>2.5                                    %jeœli moc znamionowa wiêksza ni¿ 2.5 MVA 
 Xt2=Zt2;
...
if handles.FAZ3==1                                 %jeœli mam linie trójfazow¹ p³ask¹ 
bsr=(((handles.b^2)*2*handles.b)^(1/3))*1000;      %Œredni odstêp miêdzy przewodami dla linii trójfazowych , 
else  
    bsr=handles.b*1000;% jeœli jedno fazowa   % mo¿na jeszcze dodaæ  3 uk³ad ale to jest narazie wersja testowa
%else bsr=0;
end                                                %jednotorowych  o niesymetrycznym uk³adzie przewodów *1000 bo zamienia
r=sqrt(handles.s/pi);                             %promieñ przewodów  
...
end%indukcyjnoœæ robocza jednej fazy w [H/km] /dla AFL
%ZMIENNE POMOCNICZE 
...
%funkcje opisuj¹ce elementy struktury handles , (funkcje poszczególnych
%elementów)

function Sn1_Callback(hObject, ~, handles)
num = str2double(get(hObject,'String'));
str=(get(hObject,'String'));
 if isnan(num)|str=='i'|str=='j'  %jeœli podam litere, lub znak lub zespolona wart to wyrzuci b³¹d 
     num = 0;
     set(hObject,'String',num);
     errordlg('B£ÊDNE DANE! WPROWAD PONOWNIE', 'Error')
 end
 handles.Sn1 = num;
 guidata(hObject,handles)
...
num = str2double(get(hObject,'String'));
str=(get(hObject,'String'));
 if isnan(num)|str=='i'|str=='j'  %jeœli podam litere, lub znak lub zespolon¹ wartoœæ to wyrzucam error 
     num = 0;
     set(hObject,'String',num);
     errordlg('B£ÊDNE DANE! WPROWAD PONOWNIE', 'Error')
 end
