/**
  ******************************************************************************
  * @file    02_ConfigurationReceptionTransmission/main.c 
 ...
 ===============================================================================
   - Connect a wire between PB10 to PB10 of an other discovery board
     with the example 01 loaded, there is a 4K7 PU on the line
...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define ADDRESS_INITIATOR (0x1) /* Own address */
#define ADDRESS_DESTINATION (0x0) /* Destination */
...
__INLINE void Configure_CEC(void)
{
  ...
  /* (1) CEC enable */
  /* (2) OAR = 0x002 => OA = 0x1 */
  /* (3) Receive byte interrupt enable, receive end interrupt enable */
  CEC->CR = CEC_CR_CECEN; /* (1) */
  CEC->CFGR = (0x0002<<16); /* (2) */
  CEC->IER = CEC_IER_RXBRIE|CEC_IER_RXENDIE; /* (3) */
...
void EXTI0_1_IRQHandler(void)
{
  ...
  /* (1) Set transmit IT */
  /* (2) Fill transmit data register with nibbles (initiator 0x1, destination 0x0) */
