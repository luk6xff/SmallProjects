/**
  ******************************************************************************
  * @file    10_DualMode_SimultaneousHWTrigger/main.c 
  * @author  MCD Application Team
  * @version V1.0.0
  * @date    05-March-2014
  * @brief   This code example shows how to configure the DAC in dual mode 
  *          in order to generate two simultaneous signals, the two channels  
  *          are triggered by the same timer.
 ===============================================================================
                    #####       MCU Resources     #####
 ===============================================================================
   - RCC
 ...
 ===============================================================================
    - this file must be inserted in a project containing  the following files :
      o system_stm32f0xx.c, startup_stm32f072.s
      o stm32f0xx.h to get the register definitions
      o CMSIS files
    - in the option of the project STM32F072 preprocessor symbol must be defined 
      instead of STM32F0XX
 ...
 ===============================================================================
    ...
    - Each sinewave has the same frequency by using TIM7 to trigger both DAC 
      channels.
    - The data are first computed and stored in an array. Then the DMA
      ...
      to the DAC dual register by the DMA.
    ...
    - Each sinewave has the same number of step defined by INCREMENT1.
...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* following defines are the step to generate the sinewaves, must be a divider of 90.
   The lower INCREMENTx, the lower the DACx sinewave frequency. */
#define INCREMENT1 5
#define SIN1_ARRAY_SIZE 360/INCREMENT1

/* following defines are voltages expressed in microvolts */
...
uint32_t sin1_data[SIN1_ARRAY_SIZE]; //table containing the data to generate the sinewaves on DAC ch 1 and 2 
...
/**
  ...
  *         and configures the DAC to be ready to generate a signal on both DAC1_OUT
  *         and DAC2_OUT synchronized by TIM7 HW trigger. 
  *         The signal is loaded by a DMA transfer in the dual register.
  *         The output buffer is enabled on both channel1 and channel 2.
  *         The interrupts on DMA underrun on ch 1 is enabled. 
  * @param  None
  * @retval None
  */
__INLINE void  ConfigureDAC(void)
{
  ...
  DAC->CR |= DAC_CR_TSEL1_1 | DAC_CR_TEN2 | DAC_CR_EN2 \
           | DAC_CR_TSEL2_1 | DAC_CR_DMAUDRIE1 | DAC_CR_DMAEN1 | DAC_CR_TEN1 | DAC_CR_EN1; /* (2) */   
  ...
  DAC->DHR12RD = (uint32_t)((2048 << 16) + 2048); /* Initialize the DAC value */
}
...
/**
  * @brief  This function configures the DMA channel 3 to load a value 
  *         from sin1_data[] to the DAC dual register 
...
void DMA1_Channel2_3_IRQHandler(void)
{
  ...
  {
    error |= ERROR_DMA1_XFER; /* Report error on DMA transfer */
...
void TIM6_DAC_IRQHandler(void)
{
  ...
  {
    error |= ERROR_DAC_DMA1_UNDERRUN; /* Report error on DMA underrun for the DAC */
