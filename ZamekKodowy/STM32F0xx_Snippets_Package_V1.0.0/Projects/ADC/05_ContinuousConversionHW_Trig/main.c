...
/**
  ...
  *         in continuous mode.
  *         The sequence is launched by rising edge on TIM15 external trigger.
...
__INLINE void ConfigureADC(void)
{
  ...
  /* (2) Select the external trigger on TIM15_TRGO and rising edge, 
         the continuous mode and scanning direction */
...
{
  if ((ADC1->ISR & (ADC_ISR_EOC | ADC_ISR_EOSEQ | ADC_ISR_OVR)) == 0)  /* checks if one the expected flag is set */
  {
  ...
  else
  {
    if ((ADC1->ISR & ADC_ISR_OVR) != 0)  /* checks OVR has triggered the IT */
    {
      ...
      GPIOC->BRR = (1 << 9); /* switch off green led to report event occurence  */
