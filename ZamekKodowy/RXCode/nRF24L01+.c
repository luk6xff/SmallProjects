...
void nRF24L01_SPI_COMMAND(uint8_t* regData, int nbrOfBytesToReadWrite,
	...
	case WRITE:
		...
		break;
	case READ_PAYLOAD:
		tempBuf[0] = nbrOfBytesToReadWrite+1; // number of bytes to be read + status
		tempBuf[1] = command;
		(*SPI_ReadDataCallback)(tempBuf);
		break;
	case WRITE_PAYLOAD:
...
/**
 * @name  nRF24L01_SendReadFromFifo(uint8_t*data)
 * @brief
 * @retval
 *
 */
void nRF24L01_ReadFromFifo(void) {
	nRF24L01_SPI_COMMAND(NULL, NUMBER_OF_BYTES_IN_FIFO, 0, READ_PAYLOAD);
...
void nRF24L01_Init(void) {
	if (IRQ_PinSetupCallback != NULL) {
		(*IRQ_PinSetupCallback)();   // set extrernal interrupt for IRQ pin
	}
	nRF24L01 obj;
	obj.CONFIG.PWR_UP = SET;
	obj.CONFIG.MASK_MAX_RT = SET;
	obj.CONFIG.MASK_RX_DR = RESET;
	obj.CONFIG.MASK_TX_DS = SET;
	obj.CONFIG.PRIM_RX = SET; // Rx Mode
	...
	obj.RF_CH.RF_CH = 40;
	obj.RX_PW_P0.RX_PW_P0 = NUMBER_OF_BYTES_IN_FIFO;
	...
	nRF24L01_SetCE_PIN(); // setup Rx MODE
}
