...
__INLINE void Configure_TSC(void)
{
  ...
  /* (2) Disable hysteresis, TSC_IOHCR_G2IO4 = 0, TSC_IOHCR_G2IO3 = 0 */
  /* (3) Enable end of acquisition IT, TSC_IER_EOAIE = 1 */
  /* (4) Sampling enabled, G2IO4, TSC_IOSCR_G2IO4 = 1 */
  /* (5) Channel enabled, G2IO3, TSC_IOCCR_G2IO3 = 1 */
  /* (6) Enable group, G2, TSC_IOGCSR_G2E = 1 */
  TSC->CR = (5<<12) | (1<<28) | (1<<24) | (6<<5) | (1<<0); /* (1) */
  TSC->IOHCR &= (uint32_t)(~((1<<6)|(1<<7))); /* (2) */
  TSC->IER = (1<<0); /* (3) */
  TSC->IOSCR = (1<<7); /* (4) */
  TSC->IOCCR = (1<<6); /* (5) */
  TSC->IOGCSR |= (1<<1); /* (5) */
