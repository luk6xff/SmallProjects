...
#define USART_TXRXBUFFERSIZE   (33)
...
//uint8_t SPI_TxBuffer[SPI_TXRXBUFFERSIZE], SPI_RxBuffer[SPI_TXRXBUFFERSIZE];
...
//register all necessary callbacks
	if (!nRF24L01_RegisterCallbacks(SPI2WriteDataViaInterrupt,
...
void SPI2WriteDataViaInterrupt(uint8_t*data) {
	...
	/*
	 DEBUG("LentoWrite:");  //FOR DEBUG
	 vSendIntToUart(TxData.uiLengthOfTxData); //FOR DEBUG
	 DEBUG("LentoRead:");
	 //FOR DEBUG
	 vSendIntToUart(TxData.uiLengthOfRxData); //FOR DEBUG
	 DEBUG("Tx.Message:");
	 //FOR DEBUG
	 vSendIntToUart(TxData.uiMessage[0]); //FOR DEBUG      //TU ZWIS !
	 vSendIntToUart(TxData.uiMessage[1]); //FOR DEBUG
	 */
...
void SPI2ReadDataViaInterrupt(uint8_t*data) {
	...
	DEBUG("LentoWrite/Read:");
	//FOR DEBUG
	vSendIntToUart(RxData.uiNumberOfTxRxData); //FOR DEBUG
	DEBUG("Tx.Message:");
	//FOR DEBUG
	vSendIntToUart(RxData.uiMessage[0]); //FOR DEBUG      //TU ZWIS !!!!!!!!!!!!!!!!!!!!!!
	vSendIntToUart(RxData.uiMessage[1]); //FOR DEBUG
...
void SPI2_IRQHandler(void) {
	...
	xQueueReceiveFromISR(xSPI2WriteQueueHandle, &SPI_TxData, 0);

	if (SPI_I2S_GetITStatus(SPI2, SPI_I2S_IT_RXNE) == SET) {
		//SPI_ReceiveData8(SPI2); //STATUS
		SPI_RxData.uiMessage[RxCount++] = SPI_ReceiveData8(SPI2);
		if (RxCount >= SPI_TxData.uiNumberOfTxRxData) {
			SPI_RxData.uiNumberOfTxRxData = RxCount;
			//SPI2_RxInterruptDisable();   // TODO to remove in future
 ...
 *
 */
static void vTaskSPI2(void* pvParameters) {
...
/*
		DEBUG("LenReceived:");
		//FOR DEBUG
		vSendIntToUart(spiMessage.uiNumberOfTxRxData); //FOR DEBUG
		DEBUG("Data:");
		vSendIntToUart(spiMessage.uiMessage[0]); //FOR DEBUG

		vSendIntToUart(spiMessage.uiMessage[1]); //FOR DEBUG

*/

		//vSendDataToUart(data);
		//TODO here will be handled also all sending data to SPI
	}
}
/**
...
void vStartSPITask(unsigned portBASE_TYPE uxPriority) {
	...
	xTaskCreate(vTaskSPI2, (signed const char*)"SPI", 200, NULL, uxPriority,
			&xHandleTaskSPI);
}
...
////////////////////////////////////////////////////////////IRQ_PIN-EXTERNAL INTERRUPT///////////////////////////////////////////////////////
 ...
 // in keyboard .c  --> just do  a callback here or maybe not : )


void EXTI4_15_IRQHandler(void) {
	if (EXTI->PR & EXTI_PR_PR7) {
//		GPIOC->ODR ^= (GPIO_ODR_9); //DEBUGGER
		//GPIOC->ODR ^= (GPIO_ODR_8); //DEBUGGER
...
static void vTaskUSART1(void* pvParameters) {
	...
	const char*commands[] = { "GREEN", "BLUE", "BOTH", "PWRSET", "PWRRESET", //commands must be equal or longer than 4
			"CESET", "CERESET", "NOPP", "STATUS", "CONFIG" }; // once you've added a new command , change increase loop counter
